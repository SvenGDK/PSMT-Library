...
Public Class XMBAppProject

    Private Sub BrowseProjectPathButton_Click(sender As Object, e As Windows.RoutedEventArgs) Handles BrowseProjectPathButton.Click
        Dim FBD As New FolderBrowserDialog() With {.Description = "Please select a folder to save your homebrew project.", .ShowNewFolderButton = True}
        If FBD.ShowDialog() = Windows.Forms.DialogResult.OK Then
            ProjectDirectoryTextBox.Text = FBD.SelectedPath
        End If
    End Sub

    Private Sub BrowseELFButton_Click(sender As Object, e As Windows.RoutedEventArgs) Handles BrowseELFButton.Click
        Dim OFD As New OpenFileDialog() With {.Title = "Choose your .elf file.", .Filter = "elf files (*.elf)|*.elf", .Multiselect = False}
    ...
    Private Sub BrowseIconButton_Click(sender As Object, e As Windows.RoutedEventArgs) Handles BrowseIconButton.Click
        Dim OFD As New OpenFileDialog() With {.Title = "Choose your .ico file.", .Filter = "ico files (*.ico)|*.ico", .Multiselect = False}
        If OFD.ShowDialog() = Windows.Forms.DialogResult.OK Then
    ...
    Private Sub AdvancedSettingsButton_Click(sender As Object, e As Windows.RoutedEventArgs) Handles AdvancedSettingsButton.Click
        Dim NewAppEditor As New XMBAppEditor() With {.ProjectDirectory = ProjectDirectoryTextBox.Text, .Title = "Homebrew Ressources Editor - " + ProjectDirectoryTextBox.Text}
        ...
        If Directory.Exists(ProjectDirectoryTextBox.Text) AndAlso Directory.Exists(ProjectDirectoryTextBox.Text + "\res") Then
            If File.Exists(ProjectDirectoryTextBox.Text + "\res\jkt_002.png") Then
                NewAppEditor.Cover1Image.Source = New BitmapImage(New Uri(ProjectDirectoryTextBox.Text + "\res\jkt_002.png", UriKind.RelativeOrAbsolute))
                NewAppEditor.Cover1Image.Tag = ProjectDirectoryTextBox.Text + "\res\jkt_002.png"
            End If
            ...
            If File.Exists(ProjectDirectoryTextBox.Text + "\res\info.sys") Then
                ...
                NewAppEditor.HomebrewPublisherTextBox.Text = GameInfos(5).Split("="c)(1).Trim()
                ...
                NewAppEditor.HomebrewRegionTextBox.Text = GameInfos(18).Split("="c)(1).Trim()
            Else
