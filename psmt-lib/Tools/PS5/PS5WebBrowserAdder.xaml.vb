...
Public Class PS5WebBrowserAdder
    ...
    'Sqlite method
    Private Sub CheckButtonC(sender As Object, e As Windows.RoutedEventArgs) Handles CheckButton.Click
    ...
    'Sqlite method
    Private Sub AddButtonC(sender As Object, e As Windows.RoutedEventArgs) Handles AddButton.Click
 ...
 ('nsx_content_flag_CUSA00556','{""deleted"":true,""pushed"":false}');"
    ...
    'With system_ex r/w
    Private Sub CheckButton2(sender As Object, e As Windows.RoutedEventArgs)
        If Not String.IsNullOrEmpty(ConsoleIP) Then
            'Check if we can connect to the FTP server
            If FilesAvailable(ConsoleIP) = True Then
                'Check if the browser is still located in the Tools folder
                If Directory.Exists(My.Computer.FileSystem.CurrentDirectory + "\Tools\NPXS40169") = True Then
                    AddButton.IsEnabled = True
                    MsgBox("You can install the Web Browser now.", MsgBoxStyle.Information)
                End If
            End If
    ...
    'With system_ex r/w
    Private Sub AddButton2(sender As Object, e As Windows.RoutedEventArgs)
        Try
            Using conn As New FtpClient(ConsoleIP, "anonymous", "anonymous", 1337)
                'Configurate the FTP connection
                conn.Config.EncryptionMode = FtpEncryptionMode.None
